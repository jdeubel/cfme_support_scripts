...
require 'rubygems'
#require 'orderedhash'                  # retains insertion order for hash keys
require 'dictionary'
...
def parse_controller_line(target,instance,command_hash)
...
#                   }
#  command_hash = {"originated_at" => target.split[3],
#                  "start time" => instance.get_last_log_time,
#                  "controller" => target.split[1],
#                  "method_name" => nil,
#                  "addressed_to_server"=>nil,
#                  "duration"=> nil,
#                  "method_duration" => nil,
#                  "command" => target.split[-1].tr("[]","  ").strip,
#                  "line_number_begin"=> instance.line_count,
#                  "begins_at_offset" => instance.msg_offset,               
#                  "line_number_end"=> nil,
#                  "ends_at_offset" => nil                 
#                   }
#puts  command_hash.inspect 
#  @@hash_holder = command_hash
return command_hash
end
def parse_completed_line(target,instance,partial_hash)
#  command_hash = {"line_number_end" => instance.line_count, "ends_at_offset"=> instance.msg_offset,
#                   "duration"=>nil, "addressed_to_server" => nil
#
      partial_hash["line_number_end"] = instance.line_count
      partial_hash["ends_at_offset"] = instance.msg_offset
      partial_hash["duration"] = nil
      partial_hash["addressed_to_server"] = nil
  if /: Completed in (\d*\.\d*)[[:print:]]{1,100}\|[[:print:]]{1,100}\|[[:print:]]{1,100}\[http:\/\/(\S*?)\/\S*\]/ =~ target then 
    partial_hash["duration"] = $1
    partial_hash["addressed_to_server"] = $2
    end

#  puts partial_hash.inspect
#  pp partial_hash

  return partial_hash
end

class MiqServerLog < Array
	attr_accessor :msgid, :msg_type, :msg_time, :msg_offset, :msg_process_id, :msg_type_text, :msg_payload, :agent_name, :agent_os, :agent_version, :host_os, :host_name, :host_IP_Address
...
#	puts self.inspect
	...
	@@count += 1
        @msg_offset = offset
        if log_line.size < 3 then return @valid           
        end
...
#	@@xml_detail = node2
	...
	node2.add_attribute("process_id",@msg_processid)
...
while line_in = log_file.gets
  ...
  break if array_of_hashes.size > 20000
#   puts log_line.object_id
#  pp log_line
#  puts log_line.inspect
puts Time.now if log_line.line_count.modulo(50000) == 0
  debug_cnt += 1
...
#      elsif log_line.msg_type == "D" || log_line.msg_type == nil then next #ignore debug and invalid log lines
        ...
        _x = line_in[0,120]
        long_lines.puts("line size is #{line_in.size} for line beginning as =>'#{_x}'") # some lines are very long, so lets capture & examine
      end 
...
if /: Completed/ =~ line_in then 
  completed_hash = parse_completed_line(line_in,log_line,array_of_hashes[-1])
#  array_of_hashes[(array_of_hashes.size) -1] = array_of_hashes[(array_of_hashes.size) -1].merge(completed_hash)
   array_of_hashes[-1] = completed_hash
...
#  log_line_words = line_in.split(" ")                  # get words separately
  if /Processing/ =~ line_in and log_line.msg_type == nil then
    temp_hash = Dictionary.new 
puts temp_hash.inspect   

    temp_hash = parse_controller_line(line_in,log_line,temp_hash)
puts temp_hash.inspect     
    array_of_hashes[array_of_hashes.size] = temp_hash.dup
    if array_of_hashes[-1] == temp_hash then
#      break
    else
      puts temp_hash.inspect
...
#    array_of_hashes << parse_controller_line(line_in,log_line,temp_hash) #if the first word is Processing
    # then this is a controller method call                                                                        
  end
  if /-- : (MIQ\S*): enter/ =~ line_in then
    array_of_hashes[-1].merge!({"method_name" => $1})
  end
if /-- : (MIQ\S*): exit/ =~ line_in then 
...
array_of_hashes.each do |hash_entry|
  puts hash_entry.inspect
  puts "hash count #{counter} inspect value is #{hash_entry.inspect}"
#  pp hash_entry
  break if counter > 10
  counter += 1
end
