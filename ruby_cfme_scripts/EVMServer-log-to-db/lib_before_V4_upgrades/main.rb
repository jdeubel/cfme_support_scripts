...
$active_processes = Dictionary.new           #when a new pid is recognized create a new entry
                                       #key is PID
...
$guid_process_xref = Dictionary.new          #cross reference guid as key to process id to access $active_processes
...
#          end
      # every 10k emit a progress line into the output file
        _x = Time.new - read_loop_start
          ...
          puts " 10k lines processed in #{_x} seconds - last log line time is #{$Saved_parsed_log_line.log_raw_datetime}: evm_msgs size is #{$MiqQueue.size} entries;Performance_interval size is #{$Performance_metrics.size} entries"
          else
...
#  end
  ...
  if $base_file_name == "evm" then
    puts "$large_integer is #{$large_integer.size}"
    large_integer = File.new($diag_dir + "\\large_integer_"+ $base_file_name + ".csv","w")
    large_integer.puts "large integer"
    $large_integer.each_key do |element| large_integer.puts "#{element}" end
    large_integer.close
    $large_integer.clear
  end
#end
