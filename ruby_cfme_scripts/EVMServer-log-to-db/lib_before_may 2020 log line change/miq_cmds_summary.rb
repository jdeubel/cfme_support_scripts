...
if in_hash.class.to_s != "Hash" then
  puts "miq_cmds_summary expecting Hash as passed parm, recieved #{in_hash.class.to_s},\n\t returning without creating summary file"
  return
end

return if base_file_name != "evm"                     # only build the output file for "evm" file processing

miq_cmd_summary_file = File.new($diag_dir + '\\' +"miq_cmds_summary_#{base_file_name}\.csv","w")
