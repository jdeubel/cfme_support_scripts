...
$Id: examine_log_line.rb 20995 2010-05-18 14:50:16Z thennessy $
=end
$Startup_yml_text = ""
def examine_log_line(log_line)
#  if /MIQ\(Storage.perf_capture\)/  =~ log_line  then
...
#MIQ(Host.perf_capture_queue) Skipping
    when /perf_capture_queue/ then
      return if /Skipping/ =~ $Parsed_log_line.payload                         # elminate processing for unuseful log line
    when /perf_capture_time/ then return
...
#      end
    ...
    when /BottleneckEvent\-generate_future_events/
    when /Vm\-registerVm/
