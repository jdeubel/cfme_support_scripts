...
$Id: process_vc-refresher_log_lines.rb 20948 2010-05-14 21:36:09Z thennessy $
=end
$Hosts = Hash.new
$VMs = Hash.new
$Folders = Hash.new
...
class EMS_inventory
  attr_accessor :emsname, :server_guid, :server_name, :inventory_type, :count, :log_time, :startup, :pid, :zone
  @emsname == nil
  @server_guid = nil
  @server_name = nil
  @inventory_type = nil
...
# MIQ(VcRefresher.get_vc_data) EMS: [Virtual Center (10.200.16.206)] Retrieving Storage inventory...Complete - Count: [83]
  if /MIQ\(VcRefresher\.get_vc_data\)\s*EMS\:\s*\[(.*)?\]\s*Retrieving\s*(\S*)\s*inventory...Complete\s*\-\s*Count\:\s*\[(\d*)\]/ =~ log_line then
    @emsname = $1
    @inventory_type = $2
    @count = $3
