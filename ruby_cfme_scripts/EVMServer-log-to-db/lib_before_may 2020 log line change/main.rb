...
# 
=begin rdoc
$Id: main.rb 20995 2010-05-18 14:50:16Z thennessy $

=end
# SVN Doc Info
$SVN_rev = "$Revision$ "
$SVN_author = "$Author$ "
$SVN_changed = "$Date: 2010-05-18 09:50:16 -0500 (Tue, 18 May 2010) $"
$SVN_id = "$Id: main.rb 20995 2010-05-18 14:50:16Z thennessy $"
...
#  end
 ...
 $Events_csv.close if $Events_csv != nil
  ...
  if $EMS_inventory.size > 0 then
    ...
    $EMS_inventory_csv.puts "Server Host,server GUID,startup,zone,log date time,PID,EMS name,object type,count"
#  end
  $EMS_inventory.each do |element|
    $EMS_inventory_csv.puts "#{element.server_name},#{element.server_guid},#{element.startup},#{element.zone},#{element.log_time},#{element.pid},#{element.emsname},#{element.inventory_type},#{element.count}"
  end
  end
