...
# 
=begin rdoc
$Id: main.rb 20352 2010-04-16 02:53:24Z thennessy $

=end
# SVN Doc Info
$SVN_rev = "$Revision$ "
$SVN_author = "$Author$ "
$SVN_changed = "$Date: 2010-04-15 21:53:24 -0500 (Thu, 15 Apr 2010) $"
$SVN_id = "$Id: main.rb 20352 2010-04-16 02:53:24Z thennessy $"
...
#                  "total_time,vim_connect,vim_execute_time,perf processing,pid"
    $performance_metrics_interval_file.puts  "server_guid,hostname," +
                  ...
                  "rollup perfs,db update perf,process bottleneck"
...
#      end
  #    next if /^INFO:/ =~ sorted_hash[sorted_hash.size - i][0]
      $Error_summary_file.puts "   #{sorted_hash[sorted_hash.size - (1+i)][1]}\t    #{sorted_hash[sorted_hash.size - (1+i)][0]}"
      _print_cnt += 1
...
#  end
 ...
 $Caught_event.clear
 $Events_csv.close if $Events_csv != nil

  # Make sure all other possible open files are explicitly closed
  $queue_counts_by_table.close if $queue_counts_by_table != nil
  ...
  if $EMS_inventory.size > 0 then
    ...
    $EMS_inventory_csv.puts "Server Host,server GUID,startup,log date time,PID,EMS name,object type,count"
#  end
  $EMS_inventory.each do |element|
    $EMS_inventory_csv.puts "#{element.server_name},#{element.server_guid},#{element.startup},#{element.log_time},#{element.pid},#{element.emsname},#{element.inventory_type},#{element.count}"
  end
  end
