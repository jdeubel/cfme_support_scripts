...
$Id: capture_ems_performance_interval_metrics.rb 20352 2010-04-16 02:53:24Z thennessy $
=end
...
attr_accessor :db_find_counters, :rollup_perfs, :db_update_perfs, :process_bottleneck, :db_update_perf
...
#
  if /MIQ\((.*)?\)\s*\[(\S*)\]\s*(Processing|Rollup)\s*for\s*(\S*)\s*name\:\s*\[(.*)?\],\s*id\:\s*\[(\d*)\](.*)\s*Timings\s*\{(.*)?\}/ =~ parsed_log_line.payload then
      @taskid = ""
      @miq_cmd = $1
      @interval = $2
      @target_class = $4
