...
#            _tmp = _tmp_string.join(",")
#          end
            log_msg_info_instance.msg_args = selected_args($1)      #_tmp.tr('"','\'') if _tmp != "[[]]"  # same the args data but alter all internal double quotes to single quotes
       end
     end
...
#[----] I, [2009-04-03T00:52:12.476672 #4725]  INFO -- : MIQ(EventCatcher) EMS [10.233.71.130] as [svc_miq] Queueing event [VmResourceReallocatedEvent] chainId [1637224]
  ...
  when /Synchronizing configuration/ then
  when /Synchronizing active roles/ then
  when /Garbage collection/ then
  when /Initializing DRb Connection/ then
  when /Event Monitor Thread terminated normally/ then
