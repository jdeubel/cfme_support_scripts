...
$Id: examine_log_line.rb 17090 2009-11-11 21:03:22Z thennessy $
=end

def examine_log_line(log_line)
#  if /Job created\:/  =~ log_line  then
...
###
    ...
    when /MiqServer-status_update/ then
      if /Process info/ =~ _payload.miq_post_cmd then
...
#MiqFaultTolerantVim-_connect  
    when /MiqFaultTolerantVim-_connect/     then return
...
#      if /GUID\s*\[(.*)\]\s*being killed/ =~ _payload.miq_post_cmd
       if $La_50 =~ _payload.miq_post_cmd then
        ...
        if $guid_process_xref.has_key?(_temp_pid_guid) then
          ...
          $active_processes[_target_pid]["killed"] = $Parsed_log_line.log_datetime_string      #x.log_datetime
...
#    if /with PID\s*\[(\d*)\].*, requesting worker to exit/ =~ _payload.miq_post_cmd then
    if $La_51 =~ _payload.miq_post_cmd then
...
#MIQ(WorkerMonitor) Worker [MiqGenericWorker] with PID [5066] GUID [45d6f770-439e-11de-9d68-0050568026c2] is being stopped because system resources exceeded threshold, it will be restarted once memory has freed up
    if /is being stopped/ =~ _payload.miq_post_cmd then
...
# ---------------------------------------------------------------------      
    when /proxy-call_ws/ then
      puts "#{__FILE__}:#{__LINE__}- proxy-call_ws encountered"
    when /vm-registerVm/
    ...
    when /VimPerformanceHelper-vim_collect_perf_data/ then
#      case x.payload
...
#    end
    ...
    _temp_array.each {|fragment|
      ...
      when /(\d*)\s*(\S*)\s*(\[.*\])/ then
        ...
        if $Rails_transaction["session_id"] != nil then               #if there is a session id 
          ...
          if /\#form_field_changed/ =~ $Rails_transaction["route_to"] then # if this is a console
  ...
  when /Processing/
    ...
    if /Processing\s*(.*)\s*\(for\s*(.*)\s*at\s*(.*)\)\s*\[(\S*)\]/ =~ x.payload then
      ...
      $Rails_transaction["console or web service"] = nil
      #     puts "wo"
      #      puts "#{__FILE__}:#{__LINE__}"
...
#        _worker_part2_array = _worker_part2.split(",")
     ...
     $active_processes[_worker_stats_hash["worker_pid"]] = {"PID" => _worker_stats_hash["worker_pid"] ,
       "first seen" => $Parsed_log_line.log_datetime_string ,"first seen seconds" => $Parsed_log_line.log_datetime,
       "last heard from" => $Parsed_log_line.log_datetime_string , "last heart from seconds" => $Parsed_log_line.log_datetime,
       "process type" => _worker_stats_hash["worker_type"],
...
#  puts "#{__FILE__}:#{__LINE__}->#{parsed_log_line.inspect}"
  $active_processes[parsed_log_line.log_pid]["worker type"] = parsed_log_line.payload.split[0]
  $active_processes[parsed_log_line.log_pid]["process type"] = parsed_log_line.payload.split[0]
