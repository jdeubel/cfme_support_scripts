...
#    puts "#{__FILE__}:#{__LINE__}- #{payload.miq_post_cmd}"
#  if payload.class.to_s == "NilClass" then
#    puts "#{__FILE__}:#{__LINE__}- input payload is nil"
#  end
...
#    puts "#{__FILE__}:#{__LINE__}- payload.miq_post_cmd is '#{payload.miq_post_cmd}'"
    return
  end
  save_large_integer_value(x.target_id) if x
  if x then
#    puts "$vcrefresher_refresh_array is of class '#{$vcrefresher_refresh_array.class.to_s}'"
#    puts "$vcrefresher_refresh_array_index is of class '#{$fcrefresher_refresh_array_index.class.to_s}'"
#    puts "value of x is #{x}"
    $vcrefresher_refresh_array[$vcrefresher_refresh_array_index] =  x             # only if the temp holder is non-nil
    $vcrefresher_refresh_array_index += 1
  end
end
