...
$Id: log_line_summarize.rb 16987 2009-11-05 14:25:21Z thennessy $
=end
def log_line_summarize(one_string)
#    if /353446 \#12761\] ERROR\]/ =~ one_string then
#    puts "#{__FILE__}:#{__LINE__}=>#{one_string}"
#  end
  if /Stopping Broker/ =~ one_string then
    puts "#{__FILE__}:#{__LINE__}=>#{one_string}"
  end
...
#  puts "#{one_string}"
  ...
  _parsed_one_string = Parsed_log_line.new(one_string)
...
#    if /with PID \[(\d{1,5})\] process memory usage \[(\d{1,11})\] exceeded limit \[(\d{1,12})\], restarting worker/ =~ _original_payload then
   ...
   if /has not responded in (.*) seconds/ =~ _original_payload then
       _original_payload = $PREMATCH + "has not responded in ####.### seconds" + $POSTMATCH
       line_array[1] = _original_payload
   end
...
#[----] I, [2009-02-26T20:07:57.317533 #18682]  INFO -- : MIQ(MiqWorkerMonitor-stop) Stopping worker: PID [18167], GUID [65a360e6-043a-11de-b176-005056807778], queue_name []...
  ...
  if /Stopping worker\:/ =~ _original_payload || 
    ...
    capture_stopping_worker_request()
    return
...
#        _payload = line_array[1].split       
  ...
  if /ERROR|WARN/ =~ severity && /MIQ\(/ =~ line_array[1] then
    case line_array[1]
    when /on target \[(.*)\],/ then 
  ...
  if severity !='INFO' && /storage_dispatcher_context/ !~ line_array[1] && /(.*\[)/ =~ _payload[0]  then
#    puts "#{one_string}"
...
#      $active_processes[_target_pid]["requested_exit"] = $Parsed_log_line.log_datetime_string # populate request to exit time
        ...
        puts "#{__FILE__}:#{__LINE__}-> Stopping non-existent guid-pid not recorded=>'#{$Parsed_log_line.inspect}'"
      end
...
def capture_process_stop_request(pid)
  ...
  if $active_processes.has_key?(pid) then
    $active_processes[pid]["requested_exit"] = $Parsed_log_line.log_datetime_string if $active_processes[pid]["requested_exit"] == nil  # populate request to exit time
