...
#    puts "unix name is #{x.entry_name}, entry type is '#{x.entry_type}'"
    if x.entry_type == 'dir' then
      ...
      if most_recent_current_archive.class.to_s != "NilClass" &&
         x.file_modified_time.strftime("%m/%d/%Y") == most_recent_current_archive.strftime("%m/%d/%Y") then
        ...
        if (!File.exists?(x.entry_name) || File.mtime(x.entry_name)!= x.file_modified_time) then
            if x.file_modified_time > $transfer_date_limit || !$transfer_date_limit then
                $log.info "starting binary transfer of '#{x.entry_name}'"
                 ftp_handle.getbinaryfile(x.entry_name)             # download file from ftp
                 File.utime(0,x.file_modified_time,x.entry_name) if !$model_only   # change date and time on local file system to match ftp time
                 $log.info "transfer of '#{x.entry_name}' is complete"
            else
              $log.info "file #{x.entry_name} skipped because file date #{x.file_modified_time} is before limit date #{$transfer_date_limit}"
            end     
        else
...
$transfer_option = nil
$model_only = nil                       # set to true if you only want to model the transfer and not really do it
...
if ARGV.size == nil || ARGV.size == 0 then
    ...
    if $transfer_option == "today" then $transfer_option = DateTime.now end     # if today is specified, then only download files uploaded today
    if $transfer_option == "." then $transfer_option = DateTime.now - 7 end  # if using default then only go back seven days
    _temp = Date.strptime($transfer_option, "%m/%d/%Y")
    $transfer_date_limit = Time.mktime(_temp.year,_temp.month, _temp.mday)
    puts "$transfer_date_limit is of class '#{$transfer_date_limit.class.to_s}'' with a value of '#{$transfer_date_limit}'"
