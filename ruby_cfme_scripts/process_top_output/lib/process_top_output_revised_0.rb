...
#  puts "#{__FILE__}:#{__LINE__}"
  ...
  next if linein.size <4 || linein.strip.size < 10 || linein_array.size <5
  puts "#{__FILE__}:#{__LINE__}-#{linein}"
#  if /timesync/ =~ linein then
#    puts "#{__FILE__}:#{__LINE__}=>#{linein}"
 # end
  case linein
# see if this is a detail record first before checking summary type data
  when /^\s*\d[0-9]{1,5}/
      if /^top/ =~ linein then 
...
#    linein_array = linein.split
    ...
    when 0 then
       puts "#{__FILE__}:#{__LINE__}-$Top_form = 0, #{linein_array.inspect}"
...
#        $PID_array[PID_UP_TIME] = $Top_Summary[UP_TIME]
     when 1 then
       puts "tom form = 1, #{linein_array.inspect}"
        $PID_array[0] = $Top_Summary[0] #time of day field
        $PID_array[PID_ID] = linein_array[0]
