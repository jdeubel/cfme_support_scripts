...
# the uncompressed vim.log files are way too large to keep around (tch April 2011)
  ...
  if /\.gz/ =~ _line[-1] && /evm|production|top_output|mongrel\.3000|vmstat_output|serverlog/ =~ _line[-1] then
...
_temp_array =[$evm,$production,$top_output,$mongrel,$vmstat,$vim,$serverlog]
...
##     # create a new directory for each of the appliance names
     ...
     # using 7-Zip command
     selected_files = ""
     ["evm","production","top_output","mongrel.3000","vmstat_output","vim","serverlog"].each do |type|
         case type
...
#     end
          ["evm","production","top_output","mongrel.3000","vmstat","vim","serverlog"].each do |_y|
          case _y
              when "vmstat" then
