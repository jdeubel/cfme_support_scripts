...
case log_line
#    when /Refresher(\#|\.)get_vc_data\)\s*EMS\:\s*\[(.*?)\]\,\s*id\:\s*\[(\d*)\]\s*Retrieving\s*(\S*)\s*inventory...Complete\s*\-\s*Count\:\s*\[(\d*)\]/ then
    when /EMS\:\s*\[(.*?)\]\,\s*id\:\s*\[(\d*)\]\s*Retrieving\s*(\S*)\s*inventory...Complete\s*\-\s*Count\:\s*\[(\d*)\]/ then    
      @emsname = $1
      @emsid = $2
      @inventory_type = $3
      @count = $4
...
#    when /Refresher(\#|\.)get_vc_data\)\s*EMS\:\s*\[(.*?)\]\s*Retrieving\s*(\S*)\s*inventory...Complete\s*\-\s*Count\:\s*\[(\d*)\]/ then
    when /EMS\:\s*\[(.*?)\]\s*Retrieving\s*(\S*)\s*inventory...Complete\s*\-\s*Count\:\s*\[(\d*)\]/ then    
      @emsname = $1
      @inventory_type = $2
      @count = $3
#      @log_time = $Parsed_log_line.log_datetime_string.split(".")[0]
...
#MIQ(ResourcePool-save_ems_inventory) EMS: [Virtual Center (10.233.71.130)] Updating ResourcePool [Tier2] id: [9]
  ...
  when /storage_files/ then  # since I don't know what to do with this, just catch it to prevent message in log
  when /networks/ then       # since I don't know what to do with this, just catch it to prevent message in log
  when /guest_applications/ then # since I don't know what to do with this, just catch it to prevent message in log
  when /guest_devices/ then # since I don't know what to do with this, just catch it to prevent message in log  
  when /switches/ then # since I don't know what to do with this, just catch it to prevent message in log
  when /disks/   then            # since I don't know what to do with this, just catch it to prevent message in log
  else
