...
=end
$Skip_log_lines = nil                      # set control switch to bypass processing new unknown non-conforming text in log file
puts "setting control to skip csv lines embedded in evm.log file - #{$Skip_log_lines.inspect}"
...
$process_inactive_harvest_time = 3600  # if a process has no log line activity for this number
                                       # of seconds, then declare the process as "gone" and harvest
                                       # it from the ranks of active processes

$Caught_event = Array.new              # Array of hashes to capture all recognized events

$heartbeats = Hash.new                 # capture heartbeat info
$active_processes = Hash.new          #when a new pid is recognized create a new entry, key is PID
$active_process_limit = 300           # CONTROLS when all of the active process entries in hash are dumped
default_float = "0.00".to_f

$miqserver_heartbeats_array_index = 0              #add index to reduce search time
$miqserver_heartbeats_array = Array.new
_debug = true
if _debug then
  $vcrefresher_refresh_array = Array.new(100,nil)
#  $miqserver_heartbeats_array = Array.new(210,nil)
  $large_integer = Array.new(5000,nil)  # start with 5 million instances  to see how big this gets.

else 
  $vcrefresher_refresh_array = Array.new(10000){|x| nil}  # establish array for vc_refresh timings
#  $miqserver_heartbeats_array = Array.new(100000){|x| nil} # establish array for miqserver heartbeats to be saved
...
#    case $special_debug
      ...
      if File.exist?("evm_error_pids.csv") && File.stat("evm_error_pids.csv").size? then    # if file exists and size > 0 then
        ...
        $Error_pids = $Error_pids_file.gets.chomp.split(",")
        (0..$Error_pids.size-1).each do |x|
                                      $Error_pids[x] = $Error_pids[x].strip
                         end
       puts "$Error_pids size is #{$Error_pids.size}"                        
      end
      if !$Error_pids then
          puts "$Error_pids array is empty"
      end
...
#    _d2 = Dir.mkdir($diag_dir)
  ...
  $EVMServerPid = nil                           #ensure that value is reset for each new file
    ...
    $modified_log = File.new($diag_dir + $directory_separator + "modified_" + base_file_name + ".log","w")
    $log_lines_of_interest = File.new($diag_dir + $directory_separator + "log_lines_of_interest_" + base_file_name + ".log","w")
    $Error_log = File.new($diag_dir + $directory_separator + "Errors_" + base_file_name + ".txt", "w")
    $Fatal_log = File.new($diag_dir + $directory_separator + "Fatal_" + base_file_name + ".txt","w")
    $Error_summary_file = File.new($diag_dir + $directory_separator + base_file_name + "_Error_Summary.txt","w")
    $Jobsteps_csv = File.new($diag_dir + $directory_separator + "Job_steps_" + base_file_name + ".csv","w")
    ...
    $Process_statistics_csv = File.new($diag_dir + $directory_separator + "Process_statistics.csv","w")
    ...
    $performance_metrics_interval_file = File.new($diag_dir + $directory_separator + 'performance_interval.csv',"w")
...
#                  "total_time,vim_connect,vim_execute_time,perf processing,pid"
    ...
    $performance_metrics_realtime_file = File.new($diag_dir + $directory_separator + 'performance_realtime.csv',"w")
    ...
    $Events_csv = File.new($diag_dir + $directory_separator + base_file_name + "_events.csv","w")
    ...
    $miqserver_heartbeats_file = File.new($diag_dir + $directory_separator + 'miqserver_heartbeats.csv',"w")
...
#       $vcrefresher_timings_file
    $vcrefresher_timings_file = File.new($diag_dir + $directory_separator + 'vcrefresher_timings.csv',"w")
...
#       puts "#{__FILE__}:#{__LINE__}- #{linein}"
      ...
      if /\<\<ACTIVITY_STATS_CSV/ =~ linein then
        $Skip_log_lines = true
        next
        end
      if /^ACTIVITY_STATS_CSV/ =~ linein then
      ...
      next if /ACTIVITY_STATS_CSV/ =~ linein                              # new set of log lines!?@#$!!! -skip them they mean nothing to me 
      #
      if /large payload/ =~ linein then
        next                        
      end
      if /Delivered in/ =~ linein && /Q\-task_id\(\[service_/ =~ linein then
        puts "#{linein}"
      end
...
#          end
      # every 10k emit a progress line into the output file
        ...
        cumulative_time += _x                    #accumulate total processing time
        cumulative_lines += 10000                #cumulative line count
          if $Saved_parsed_log_line.class.to_s == "Parsed_log_line" then
          puts " 10k lines processed in #{_x} seconds - last log line time is #{$Saved_parsed_log_line.log_raw_datetime}: \n\tcumulative processing time is #{cumulative_time.to_s.gsub(/(\d)(?=\d{3}+(?:\.|$))(\d{3}\..*)?/,'\1,\2')} seconds, cumulative line count is #{cumulative_lines.to_s.gsub(/(\d)(?=\d{3}+(?:\.|$))(\d{3}\..*)?/,'\1,\2')};" +
               "\n\tevm_msgs size is #{$MiqQueue.size} entries;\n\tPerformance_interval size is #{$Performance_metrics.size} entries;"  +
               ...
               "\n\t$large_integer size is #{$large_integer_index.to_s.gsub(/(\d)(?=\d{3}+(?:\.|$))(\d{3}\..*)?/,'\1,\2')}"
          else
...
#      end
      if linein.size > (4096 * 10) then
        ...
        _pre_truncate_line_size = linein.size
        linein = linein[0,1024]  
        linein = linein.to_s << " ... #{_pre_truncate_line_size} character log line truncated at 1k "
#        puts "line limit exceeded=>#{linein}"
#        next
      end
  if /\[Broker\]\s*Connected/ =~ linein then
#    puts "#{__FILE__}:#{__LINE__}- #{linein}"
  end
        case linein
...
#  end
  if $Startups.size > 0 then
                  startups_file = File.new($diag_dir + $directory_separator + "startups_#{base_file_name}\.csv","w")
  ...
  #    next if /^INFO:/ =~ sorted_hash[sorted_hash.size - i][0]
    ...
    if !$Job_cmds.empty? && FALSE then
      ...
      puts "\n\nJob Command summary follows for #{$Job_cmds.size} entries\n\t"  
        ...
        $Job_cmds_csv = File.new($diag_dir + $directory_separator + "Jobs_CSV_" + base_file_name + ".csv","w")
...
#  ===
        process_synopsis = File.new($diag_dir + $directory_separator + "process_synopsis_" + $base_file_name + ".csv","w")
...
#  _process = _process_array[1]
  if _process["worker type"] == nil then 
    _process["worker type"] = "EVM Unknown"
  end
...
process_synopsis = nil                                    # set handle to nil
  ...
  if $heartbeats then
    $heartbeats_csv = File.new($diag_dir + $directory_separator + $base_file_name + "_heartbeats.csv","w")
...
if $Hosts.size > 0 then
  ...
  $Hosts_csv = File.new($diag_dir + $directory_separator + $base_file_name + "_Hosts.csv","w")
...
if $VMs.size > 0 then
  ...
  $VMs_csv = File.new($diag_dir + $directory_separator + $base_file_name + "_VMs.csv","w")
...
#    data["emsname"] = "" if data["emsname"] == "_emsname"
    ...
    save_large_integer_value(data["vmid"])
    save_large_integer_value(data["hostid"])
    save_large_integer_value(data["emsid"])
    end
...
if $Folders.size > 0 then
  ...
  $Folders_csv = File.new($diag_dir + $directory_separator + $base_file_name + "_Folderss.csv","w")
...
if $ResourcePools.size > 0 then
  puts 
  $ResourcePools_csv = File.new($diag_dir + $directory_separator + $base_file_name + "_ResourcePools.csv","w")
...
if $Storages.size > 0 then
  ...
  $Storages_csv = File.new($diag_dir + $directory_separator + $base_file_name + "_Storages.csv","w")
...
if $Clusters.size > 0 then
  ...
  $Clusters_csv = File.new($diag_dir + $directory_separator + $base_file_name + "_Clusters.csv","w")
...
#  end
 ...
 $Events_csv.close if $Events_csv != nil && $base_file_name == "evm"
  ...
  if $EMS_inventory.size > 0 then
    $EMS_inventory_csv = File.new($diag_dir + $directory_separator + base_file_name + "_ems_summary_inventory.csv","w")
...
#  end
  ...
  if $base_file_name == "evm" then
    ...
    msg_server_guid_file = File.new($diag_dir + $directory_separator + "msg_server_guid_" + $base_file_name + ".csv","w")
    ...
    msg_class_name_file = File.new($diag_dir + $directory_separator + "msg_class_name_" + $base_file_name + ".csv","w")
  ...
  if $base_file_name == "evm" then
    ...
    msg_server_host = File.new($diag_dir + $directory_separator+ "msg_server_host_"+ $base_file_name + ".csv","w")
  ...
  if $base_file_name == "evm" then
    ...
    msg_processing_status = File.new($diag_dir + $directory_separator + "msg_processing_status_" + $base_file_name  + ".csv","w")
  ...
  if $base_file_name == "evm" then
    ...
    msg_ident_id = File.new($diag_dir + $directory_separator + "msg_ident_id_"+ $base_file_name + ".csv","w")
  ...
  if $base_file_name == "evm" then
    ...
    msg_task_id = File.new($diag_dir + $directory_separator + "msg_task_id_"+ $base_file_name + ".csv","w")
  ...
  if $base_file_name == "evm" then
    ...
    msg_method_name = File.new($diag_dir + $directory_separator + "msg_method_name_"+ $base_file_name + ".csv","w")
  ...
  if $base_file_name == "evm" then
    ...
    msg_data_store = File.new($diag_dir + $directory_separator + "msg_data_store_"+ $base_file_name + ".csv","w")
    ...
    if$base_file_name == "evm" then
      ...
      msg_data_msg_cmd = File.new($diag_dir + $directory_separator + "msg_data_msg_cmd_"+ $base_file_name + ".csv","w")
   ...
   if $base_file_name == "evm" then
      ...
      msg_error_text = File.new($diag_dir + $directory_separator + "msg_error_text_"+ $base_file_name + ".csv","w")
  ...
  if $base_file_name == "evm" then
    ...
    msg_role = File.new($diag_dir + $directory_separator + "msg_role_"+ $base_file_name + ".csv","w")
  ...
  if $base_file_name == "evm" then
    ...
    msg_zone = File.new($diag_dir + $directory_separator + "msg_zone_"+ $base_file_name + ".csv","w")
  ...
  if $base_file_name == "evm" then
    ...
    msg_args = File.new($diag_dir + $directory_separator + "msg_args_"+ $base_file_name + ".csv","w")
  ...
  if $base_file_name == "evm" then
    ...
    $large_integer.each do |element| 
                        $large_integer_file.puts "#{element}" if element  # write out value if it isn't nil
                        end
    end
    $large_integer_file.close
    $large_integer.clear
  end
#end
#end
