...
def complete_heartbeat
  ...
  puts "#{__FILE__}:#{__LINE__}-> Heartbeat cmpleted without hearbeat begin at \n\t#{$current_miqserver_heartbeat.end_time} "
   ...
   $miqserver_heartbeats_array[$miqserver_heartbeats_array_index] = $current_miqserver_heartbeat
    $miqserver_heartbeats_array_index += 1   # don't build up queue - just write  them out
