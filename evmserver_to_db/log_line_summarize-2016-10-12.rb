...
# MIQ(VcRefresher.get_vc_data) EMS: [Virtual Center (10.200.16.206)] Retrieving Storage inventory...Complete - Count: [83]
  ...
  when / VdlDisk\: / then
     return if _parsed_one_string.log_type_word == "WARN"
  when /dashboard_controller\-maintab/ then
    return if _parsed_one_string.log_type_word == "WARN"
  when /vm_infra_controller\-wait_for_task/ then
    return if _parsed_one_string.log_type_word == "WARN"
  when /support_controller\-index/ then
    return if _parsed_one_string.log_type_word == "WARN"
  when /vm_infra_controller\-tree_select/ then
    return if _parsed_one_string.log_type_word == "WARN"
  when /vm_infra_controller\-explorer/ then
    return if _parsed_one_string.log_type_word == "WARN"
  when /vm_infra_controller\-show/ then
    return if _parsed_one_string.log_type_word == "WARN"
  when /miq_policy_controller\-action_edit/ then
    return if _parsed_one_string.log_type_word == "WARN"
  when /vm_infra_controller\-x_show/ then
    return if _parsed_one_string.log_type_word == "WARN"
  when /vm_infra_controller-x_history/ then
    return if _parsed_one_string.log_type_word == "WARN"
  when /miq_policy_controller\-upload/ then
    return if _parsed_one_string.log_type_word == "WARN"
  when /vm_infra_controller\-reload/ then
    return if _parsed_one_string.log_type_word == "WARN"
  when /vm_infra_controller\-guest_applications/ then
    return if _parsed_one_string.log_type_word == "WARN"
  when /vm_infra_controller\-x_button/ then
    return if _parsed_one_string.log_type_word == "WARN"
  when /miq_policy_controller\-accordion_select/ then
    return if _parsed_one_string.log_type_word == "WARN"
  when /ems_cluster_controller\-show_list/ then
    return if _parsed_one_string.log_type_word == "WARN"
  when /ems_cluster_controller\-index/ then
    return if _parsed_one_string.log_type_word == "WARN"
  when /ems_cluster_controller\-protect/ then
    return if _parsed_one_string.log_type_word == "WARN"
  when /ems_cluster_controller\-button/ then
    return if _parsed_one_string.log_type_word == "WARN"
  when /miq_policy_controller\-get_json/ then
    return if _parsed_one_string.log_type_word == "WARN"
  when /miq_policy_controller\-profile_edit/ then 
    return if _parsed_one_string.log_type_word == "WARN"
  when /miq_policy_controller\-tree_select/ then
    return if _parsed_one_string.log_type_word == "WARN"
  when /miq_policy_controller\-export/ then
    return if _parsed_one_string.log_type_word == "WARN"
  when /miq_policy_controller\-import/ then 
    return if _parsed_one_string.log_type_word == "WARN"
  when /miq_policy_controller\-profile_edit/ then
    return if _parsed_one_string.log_type_word == "WARN"
#  when /miq_policy_controller\-explorer/ then
#    return if _parsed_one_string.log_type_word == "WARN"
  when /vm_infra_controller\-protect/ then
    return if _parsed_one_string.log_type_word == "WARN"
  when /ops_controller\-schedule_form_field_changed/ then
    return if _parsed_one_string.log_type_word == "WARN"
  when /ops_controller\-x_button/ then
    return if _parsed_one_string.log_type_word == "WARN" 
  when /miq_policy_controller\-explorer/ then
    return if _parsed_one_string.log_type_word == "WARN" 
  when /miq_policy_controller\-profile_field_changed/ then
    return if _parsed_one_string.log_type_word == "WARN" 
  when /miq_policy_controller\-x_button/ then
    return if _parsed_one_string.log_type_word == "WARN" 
  end
...
#     puts "#{__FILE__}:#{__LINE__}- #{_original_payload}"
     ...
     _miq_worker_type = $1
#     if _miq_worker_type == "VimBrokerWorker" then
#       puts "#{_miq_worker_type}"
#     end
...
#        puts "#{__FILE__}:#{__LINE__}- #{_original_payload}"
        if $active_processes.has_key?(_pid) then
          ...
          $active_processes[_pid]["worker type"] = _miq_worker_type
...
#        end
    ...
    if /Exit request received. Worker exiting.|Invoking shutdown method for MiqServer/ =~ _original_payload ||
        ...
        /seconds\. Worker exiting\./ =~ _original_payload ||
        /failed authentication check\. Worker exiting./ =~ _original_payload ||
...
#        $miqserver_termination_msg = _original_payload
      ...
      if /failed authentication check\./ =~ _original_payload then
        $active_processes[$line_group_pid]["requested_exit_reason"] = "failed authentication check"
      end
...
#[----] I, [2009-02-26T20:07:57.317533 #18682]  INFO -- : MIQ(MiqWorkerMonitor-stop) Stopping worker: PID [18167], GUID [65a360e6-043a-11de-b176-005056807778], queue_name []...
  ...
  if /Stopping worker\:/ =~ _original_payload || 
      ...
      /Stopping Broker/ =~ _original_payload &&
      /stopped|killed|aborted/ !~ _original_payload then
...
#      if $guid_process_xref.has_key?($1) then
      _target_pid = $1
        if _target_pid == "..." then
            if /PID\s*\[(\d*)\]\s*GUID/ =~$Parsed_log_line.payload then
              _target_pid = $1
            end
        end
...
#    line_array[1] = _original_payload
#  end
  if /rubyrep:\s*(.*)\s*Exception caught\: replication run timed out/ =~ _original_payload then
    _original_payload = "rubyrep: {specific time} Exception caught: replication run timed out"
    line_array[1] = _original_payload
  end
...
#        _payload = line_array[1].split       
  ...
  if /ERROR|WARN/ =~ severity  then
    if /MIQ\(/ =~ line_array[1] then
      case line_array[1]
      when /\.perf_init\)/ then
        if /id\:\s*\[\d*\] Failed/ =~ line_array[1] then
          line_array[1] = $PREMATCH + " id: [...] Failed" + $POSTMATCH
        end
      when /\.perf_collect_metrics\)/ =~ line_array[1] then
        if /\],\s*\[\d*\],\s*\[.*?\]\s*Unhandled/ =~ line_array[1] then
          line_array[1] = $PREMATCH + "], [...], [___] Unhandled" + $POSTMATCH
        end
      when /Error delivering\s*\{/ then
#         puts "#{__FILE__}:#{__LINE__} - original line_array[1] is '#{line_array[1]}'"
        if /\{(.*?),\s*(.*?)\}\s*for object\s*\[(.*?)\]\s*with state\s*\[(.*?)\]/ =~ line_array[1] then
#          puts "#{__FILE__}:#{__LINE__} - original line_array[1] is transformed into"
          _temp_obj_name_string = $3
          if _temp_obj_name_string.size > 0 then
            _temp_obj_name = _temp_obj_name_string.split('.')
            _temp_obj_name[0] = _temp_obj_name[0] + "..." if _temp_obj_name.size > 1
          else _temp_obj_name[0] = ""
          end
          line_array[1] = $PREMATCH + " {" + $1 + ",...} for object [" + "#{_temp_obj_name[0]}" + "] with state [" + $4 +"]" + $POSTMATCH
#          puts "#{__FILE__}:#{__LINE__} - \t #{line_array[1]}"
        else 
          if /\{(.*?)\}\s*for object\s*\[(.*?)\]\s*with state\s*\[(.*?)\]/  =~ line_array[1] then
#             puts "#{__FILE__}:#{__LINE__} - original line_array[1] is transformed into"
          _temp_obj_name_string = $2
          if _temp_obj_name_string.size > 0 then
            _temp_obj_name = _temp_obj_name_string.split('.')
            _temp_obj_name[0] = _temp_obj_name[0] + "..." if _temp_obj_name.size > 1            
          else _temp_obj_name[0] = ""
          end
            line_array[1] = $PREMATCH + " {" + $1 + "} for object [" + "#{_temp_obj_name[0]}" + "] with state [" + $3 +"]" + $POSTMATCH
#             puts "#{__FILE__}:#{__LINE__} - \t #{line_array[1]}"
          end
        end
# [----] E, [2015-11-23T16:13:53.979095 #16124:32de94] ERROR -- : 
# Q-task_id([service_template_provision_task_4000000000010]) MIQ(MiqAeEngine.deliver) Error delivering {
# "dialog_option_0_vm_name"=>"poc-tst11-23f", "dialog_option_0_vlan"=>"mgt0", "dialog_option_0_linux_domain_name"=>"poc.local", "dialog_option_0_dns_servers"=>"10.220.0.50", "dialog_option_0_flavor"=>"bronze", "dialog_option_0_respool"=>"4000000000010", "password::dialog_option_0_rootpw"=>"v2:{N4v8X5IbLZ77pXxqGZ6iLw==}", "dialog_option_0_staas_tier"=>"premium", "dialog_tag_0_dmaas_tier"=>"i_s_bus_90d", "dialog_tag_0_mon_svc"=>"p_mon_bnd1", "dialog_tag_0_mgd_svc"=>"p_mgt_vlm", "dialog_tag_0_sms_svc"=>"no", "dialog_tag_0_sms_number"=>"ac3_cloud_mgt_none", "dialog_selections_confirmed"=>"t", "request"=>"clone_to_service"
# } 
# for object [ServiceTemplateProvisionTask.4000000000010] 
# with state [checkprovisioned] to Automate:       
...
#puts "#{line_array.inspect}"
    if /INFO/ =~ severity &&                                        #examine more closely INFO lines with the word error
       ...
       /power state has changed / !~ line_array[1] &&                  # catch state changes into INFO
       /\<PolicyEngine\>/ !~ line_array[1] &&
         /ActiveRecord/ !~ line_array[1] &&
#         /MIQ\(MiqAlert\.evaluate_alerts\)/ !~ line_array &&
       /get_ems_metadata_tree/ !~ line_array[1] &&
         /Inspecting ServiceNow/ !~ line_array[1] &&
         /service_now_task_get_records/ !~ line_array[1] &&
         /\.service_now_task/ !~ line_array[1] &&
...
#      when /\<AutomationEngine\> \<User-Defined Method\>/ then
      ...
      when /catalog_controller\-dialog_field_changed/ then   
      when /catalog_controller\-x_button/ then        
      when /miq_ae_customization_controller\-dialog_form_field_changed/ then 
      when /ops_controller\-explorer/ then
      when /miq_ae_customization_controller\-tree_select/ then
      when /ops_controller\-change_tab/ then
      when /catalog_controller\-svc_catalog_provision/ then
      when /miq_ae_customization_controller\-x_button/ then
      when /catalog_controller\-dialog_form_button_pressed/ then   
      when /miq_ae_class_controller\-tree_select/ then 
      when /miq_ae_class_controller\-update_method/ then 
      when /miq_ae_class_controller\-form_method_field_change/ then 
      when /miq_ae_class_controller\-x_button/ then 
      else
  ...
  if /Garbage collection took\s*(.*)\s*seconds/ =~ line_array[1] then
    _time = $1.to_f
    _work_string = ""
    line_array[1] = $PREMATCH       #preserve what came before
#    case _time
    if _time > 600 then
    ...
    line_array[1] = line_array[1] + "Garbage collection took > #{_work_string} seconds"
  end
  ...
  if /expected to get data as of/ =~ line_array[1] then
    line_array[1] = $PREMATCH + "expected to get data as of ..."
    if /name\: \[(.*?)\], id\: \[(\d*)\]/ =~ line_array[1] then
      line_array[1] = $PREMATCH + "name: [___], id: [...]" + $POSTMATCH
    end
  end

#MIQ(MiqPerfCollectorWorker.log_destroy_of_worker_messages) Destroying: Message id: [...], MiqWorker id: [....] Zone: [London1], Role: [performancecollector], Server: [...], Ident: [performancecollector], Target id: [], Instance id: [5000000000149], Task id: [], Command: [Host.perf_capture], Timeout: [600], Priority: [100], State: [dequeue], Deliver On: [], Data: [], Args: ["realtime"]
   if /\.log_destroy_of_worker_messages\)/      =~ line_array[1] then
     if /Instance id\: \[\d*\]/ =~ line_array[1] then
       line_array[1] = $PREMATCH + "Instance id: [...]" + $POSTMATCH
     end

   end

  if /out of range/ =~ line_array[1] then
#MIQ(Vm.perf_process) [realtime] Vm name: [ccwlab-ciq01], id: [1017] Timestamp: [2011-08-25T18:32:20Z], Column [mem_usage_absolute_average]: 'percent value 102.84 is out of range, resetting to 100.0'
    if /Timestamp\:\s*\[.*?\],/  =~ line_array[1] then
      line_array[1] = $PREMATCH + "Timestamp: [...]," + $POSTMATCH  #preserve all but timestamp to allow for dups to be counted
    end
    if /value (.*?) is/ =~ line_array[1] then
      line_array[1] = $PREMATCH + "value ### is" + $POSTMATCH       #MAKE ACTUAL VALUE AN ABSTRACTION
    end
  end

#/MIQ(Vm.perf_collect_metrics) [realtime] for: [Vm], [5000000001780], [NLDN4492PAP]  Unhandled exception during perf data collection:: [Connection refused - connect(2) (https://14.67.4.25:443)], class: [Errno::ECONNREFUSED]
  if /\.perf_collect_metrics\)/ =~ line_array[1] then
    if /\], \[(\d*)\], \[(.*?)\]\s*Unhandled/ =~ line_array[1] then
      line_array[1] = $PREMATCH + "], [...], [___] Unhandled" + $POSTMATCH
    end
    if /\], \[(\d*)\], \[(.*?)\]\s*Timings at time of error\:/ =~ line_array[1] then
      line_array[1] = $PREMATCH + "], [...], [___]   Timings at time of error: ..."
    end
    if /\], \[(\d*)\], \[(.*?)\]\s*Timings before retry\:/ =~ line_array[1] then
      line_array[1] = $PREMATCH + "], [...], [___]   Timings before retry: ..."
    end
    if /\], \[(\d*)\], \[(.*?)\]\s*Timeout/ =~ line_array[1] then
      line_array[1] = $PREMATCH + "], [...], [___] Timeout" + $POSTMATCH
    end
  end
  if /Session object size of (\d*\.\d*|\d*) KB/ =~ line_array[1] then
    line_array[1] = $PREMATCH + "Session object size of ###.# KB" + $POSTMATCH
  end
...
#     initialize_evm_startups_config # added by tch 12/16/2011 to increment startups
    end
  end
...
#    }
    ...
    if $active_processes[$Parsed_log_line.log_pid]["worker type"].class.to_s == "NilClass" then
      puts "#{__FILE__}:#{__LINE__}: Worker type is not recognized\n\t#{$Parsed_log_line.log_datetime_string}\n\t#{$Parsed_log_line.payload}"
    end
...
def capture_stopping_worker_request()
  ...
  # current log line is requesting stop of a pid-guid
    if /(PID|PID\:)\s*\[(\d{1,5})\]/ =~ _temp_payload then
      _tmp_pid = $2
...
def capture_process_stop_request(pid)
  if pid == nil || pid == "" then
    puts "#{__FILE__}:#{__LINE__}- PID[#{pid}] not found in $active_processes hash - dumping log line payload for analysis" +
      "\n\t'#{$Parsed_log_line.payload}' "
    return
  end
  if pid == "..." then
...
#MIQ(WorkerMonitor) Worker [MiqGenericWorker] with PID [5066] GUID [45d6f770-439e-11de-9d68-0050568026c2] is being stopped because system resources exceeded threshold, it will be restarted once memory has freed up
        ...
        if $active_processes[pid]["requested_exit_reason"].class.to_s == "NilClass" ||
          ...
          when /uptime has reached the interval/ then
              if /MiqEvent\./  !~ $Parsed_log_line.payload then
...
#Parent MiqServer id [1] GUID [517dd17e-bea1-11de-a74c-00505691302e] has not responded in 180 seconds. Stopping Broker. 
    ...
    if /MiqEvent\./  !~ $Parsed_log_line.payload &&
       /AutomationEngine/ !~ $Parsed_log_line.payload then
    puts "#{__FILE__}:#{__LINE__}- PID[#{pid}] not found in $active_processes hash - dumping log line payload for analysis" +
