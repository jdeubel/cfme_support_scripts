...
#  || /Q\-[Tt]ask_id\(\[(.*)\]\) / =~ log_line then
  puts "#{__FILE__}:#{__LINE__}- log_line before editing \n==> #{log_line}"

  log_line = $PREMATCH + $POSTMATCH
    puts "#{__FILE__}:#{__LINE__}- log_line after editing \n==> #{log_line}"
...
#   x = Parsed_log_line.new(log_line)
    ...
    return
  when /Q-[Tt]ask_id\(\[(.*?)\]\)\s*MIQ\(MiqQueue(\.|\#)/ # =~ log_line then
    # tch 2008-12-17
...
#    x = $Parsed_log_line                    #  use the already created global object value instead
  ...
  if /MIQ\(config\) Database Adapter\: \[(.*?)\], version\:/ =~ log_line ||
      ...
      / ENV\[\'EVMSERVER\'\]\: true/ =~ log_line ||
      /MIQ\(EvmApplication.start\) EVM Startup initiated/ =~ log_line ||
...
if $EVMServerPid == nil || $EVMServerPid == x.log_pid then
    ...
    #    #    puts "#{__FILE__}:#{__LINE__}"
        ...
        $startup_config = File.new($diag_dir + $directory_separator + "startup_config_#{$startup_cnt}","w")
        capture_evm_startup_config(log_line)
      end
...
#      /MIQ\(EvmApplication\.start\) EVM Startup initiated/ =~ log_line   ||
        /MIQ\(EvmApplication.start\) EVM Startup initiated/ =~ log_line ||
      /ENV\[\'MIQ_GUID\'\]\: , ENV\[\'EVMSERVER\'\]\: true/ =~ log_line  then
        recognize_evm_startup()
        $active_job_cnt = 0
...
#MIQ(Host.perf_capture_queue) Skipping
    ...
    when /(\.|\#)perf_collect_metrics$/ then return                                # nothing of interest in this line
...
#        end
    ...
    when /(\.|\#)perf_rollup_to_parent$/ then return
    when /(\.|\#)perf_capture_queue$/ then return
    when /(\.|\#)perf_build_query_params$/ then return
    when /(\.|\#)perf_capture$/ then
      if /Timings/ =~ _payload.miq_post_cmd then
...
#      puts "#{__FILE__}:#{__LINE__}- heartbeat"
      ...
      puts "#{__FILE__}:#{__LINE__}-" + _payload.miq_post_cmd
          ...
          when /time\:\[unknown\]/ then _smartproxy_time = "unknown" 
...
#    when /MiqWorkerMonitor|MiqVimBrokerWorker|MiqGenericWorker|MiqScheduleWorker|MiqPriorityWorker|MiqEventCatcher|MiqEventHandler|MiqWorker-status_update/ then
      ...
      when /\#log_status/ then process_miqworker_status_update(_payload)
      when /MiqWorker(\.|\-)log_status/ then
        puts "#{__FILE__}:#{__LINE__}->_payload -> #{_payload}"
        process_miqworker_status_update(_payload)
...
#VcRefresher-get_vc_data
    when /Refresher[\.|\-|\#]get_vc_data/ then return
#VcRefresher-refresh
...
#                when /vCenter\:/ then _worker_stats_hash["category"] = "Virtual Center"
          ...
          puts "#{__FILE__}:#{__LINE__}\n\t_worker_part1 = '#{_worker_part1}'\n\t_worker_part2 = '#{_worker_part2}'\n\t_worker_stats_hash = #{_worker_stats_hash.insect}'\n\t x.log_datetime = '#{x.log_datetime}'"
...
#    when /MiqEmsRefreshWorker\.start/
    ...
    when /EmsRefresh\.vc_update_event/ then
      if /Queueing refresh/ =~ x.payload then
        capture_queueing_refresh(x.payload)
      end
...
#        puts "#{__FILE__}:#{__LINE__}=> Job[#{_job_uuid}] 'ready' or starting but create never recognized"
    ...
    if /Q-[Tt]ask_id\(\[.*\]\)\s*JOB\(\[(.*?)\]\s*(\S*)\)\s*(.*)$/ =~ x.payload then
      _job_uuid = $1
      _remainder = $3
      _job_cmd = $2
      if $Job_cmds.has_key?(_job_uuid) == true then
...
def cleanup_rails
  if $rails_transaction_cnt < 2 then
    ...
    $Rails_transactions_csv = File.new($diag_dir + $directory_separator + "Rails_Transactions_" + "production" + ".csv","w")
...
#       "server_guid" => "uninitialized", "server_hostname" => "uninitialized",
       ...
       _new_pid_file = nil
     _new_pid_file = File.new($pid_dir + $directory_separator +
