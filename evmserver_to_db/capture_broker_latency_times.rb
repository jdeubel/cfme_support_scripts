...
#  puts "#{__FILE__}:#{__LINE__}-#{parsed_log_line.payload}"
  #File.new($diag_dir  + $directory_separator + "db_statistics_"+ $base_file_name + ".csv","w")
  if $broker_connection_latency == nil then
    $broker_connection_latency = File.new($diag_dir  + $directory_separator + "broker_connection_latency.csv","w")
