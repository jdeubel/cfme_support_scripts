...
# V4 db stats sample
#[----] I, [2011-03-05T14:19:58.663047 #12742:1599c2c8c938]  INFO -- :
#(postgresql-log_ar_stats) interval: 60 seconds, last logged: 61.051199 seconds ago: (Sizes are estimated) {:request_max_size=>868, :responses_total_size=>405, :db_seconds=>3.5851001739502, :response_max_size=>405, :requests=>2, :max_db_seconds=>1.80816650390625, :responses=>2, :requests_total_size=>988}
    if/\((\S*)\)\-.log_ar_stats/ =~ payload then
      @db_type = $1
    end
...
def write_db_stats(db_stats)
  ...
  if $db_stats == nil then
    ...
    $db_stats = File.new($diag_dir  + $directory_separator + "db_statistics_"+ $base_file_name + ".csv","w")
