...
=end
def triage_log_line1(line_group_in_parm)
=begin rdoc
...
=end
  case line_group_in_parm.class.to_s
  when "String" then
    line_group_in = line_group_in_parm
  when "Array" then
    line_group_in = line_group_in_parm[0]
  end
...
#    if /has reached the interval/ =~ line_group_in &&
#        /MiqQueue\./  !~ line_group_in then
#     puts "#{__FILE__}:#{__LINE__}- #{line_group_in}"
#  end
  current_log_line = $Parsed_log_line
  return_value = false
  if log_lines_to_skip(line_group_in) then
    return true
  end
