...
$Id: capture_last_startup_info.rb 21179 2010-05-24 11:26:54Z thennessy $
=end
...
#      last_startup_config_info.close
  ...
  while !good_input_file
    ...
    puts "#{log_line_in}"
    if /\[----\]/ =~ log_line_in && /\-\- \:/ =~ log_line_in then
...
#    if /\_EVM\|/ =~ config_line_in then
#      puts "#{__FILE__}:#{__LINE__}"
#    end
    case config_line_in
    when /processor_count/ then
      $Startups[$startup_cnt]["processor count"] = config_line_array[5]
    when /processor_speed/ then
      $Startups[$startup_cnt]["processor speed"] = config_line_array[5]
    when /real_memory/ then
      $Startups[$startup_cnt]["real memory"] = config_line_array[5]
    when /swap_memory/ then
      $Startups[$startup_cnt]["swap memory"] = config_line_array[5]
    when /Server_id/ then
      $Startups[$startup_cnt]["Server id"] = config_line_array[5]
    when /ipaddress/ then
      $Startups[$startup_cnt]["host ipaddress"] = config_line_array[5]
    when /Server_start_datetime,/ then
      ...
      $Startups[$startup_cnt]["log_time"] = _worktime if $Startups[$startup_cnt]["log_time"] == nil ||$Startups[$startup_cnt]["log_time"] == ""
      else
...
#      if !_using_log_start_time then
      $Startups[$startup_cnt]["build"] = config_line_array[5].tr('"',' ').strip
...
##      $Startups[$startup_cnt]["role"] = $Startups[$startup_cnt]["role"].tr('"',' ').strip
#      else
