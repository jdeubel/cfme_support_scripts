BEGIN {   FS=" "
         ...
         CONVFMT="%.3f"
         OFMT="%.3f"
       print "date-time,pid-tid,messageid,Ident,queued time, element count,PRovider cnt,Host cnt,VM cnt,Template cnt,duration"
       }
...
/MIQ\(MiqQueue.delivered\)/ {  array_key = $11 
                                ...
                                if ( array_key in msgid_array) { msgid_array[array_key] = msgid_array[array_key] $(NF-1)
